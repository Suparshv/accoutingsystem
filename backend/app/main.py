...
# Uploaded contact images (SPEC.md §5 UPLOAD_DIR, §17 P1). Mounted UNDER the
# /api prefix on purpose: the frontend's VITE_API_BASE_URL already ends in
# "/api", so PartnerOut.image_url ("/uploads/<file>") is appended to the base
# it holds and needs no second origin configured anywhere.
#
# StaticFiles serves only what is inside this directory and resolves ".." out
# of the request path itself, and every name in there was generated by
# core/uploads.py rather than taken from a client.
